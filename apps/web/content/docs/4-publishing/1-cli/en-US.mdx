...
There are two ways to load a built presence without the Nowly API. Full walkthrough: [Load and Test Locally](/docs/presence-development/load-and-test).
...
If Nowly is already loaded in the browser, pack the presence and drop it in the Debug panel (developer mode):

```bash
nowly pack youtube
```

The zip is written to `dist/packs/youtube.zip`. Drop it on **Settings → Advanced → Debug**. Unsigned zips only work on unpacked builds or with developer mode enabled.
