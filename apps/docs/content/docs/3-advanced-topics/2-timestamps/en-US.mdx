...
`document` is the browser DOM API (available because presences run as content scripts). `Assets` is provided by the runtime (see [Assets](/presence-development/assets)).
