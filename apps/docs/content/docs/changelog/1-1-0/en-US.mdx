...
<ReleaseTable
  ...
  rows={[
    ...
    ["Disable flow", "Disabling a presence removes that device from the active counter.", "Stale activity does not linger after a toggle."],
    ...
    ["Changelog structure", "The changelog moved to `/changelog` and now uses one page per version.", "Each release can be linked, scanned, and maintained independently."],
