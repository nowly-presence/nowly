...
export const normalizeLocalMetadata = (raw: unknown, slug: string): PresenceMetadata | null => {
  ...
  const authorName = authorRaw && typeof authorRaw === "object" && typeof (authorRaw as { name?: unknown }).name === "string"
  ...
  return {
    ...
    runAt: data.runAt === "document_start" || data.runAt === "document_end" || data.runAt === "document_idle"
      ? data.runAt
      : "document_idle",
...
const fileAt = (lookup: Map<string, Uint8Array>, path: string): Uint8Array | undefined =>
  lookup.get(normalizePath(path).toLowerCase());
...
export const parsePresenceZip = async (
  ...
  try {
    files = Object.fromEntries(
      Object.entries(unzipSync(bytes)).map(([path, content]) => [normalizePath(path), content]),
    );
  } catch {
  ...
  const fallbackSlug = fileName.replace(/\.zip$/i, "").toLowerCase().replace(/\s+/g, "-") || "local-presence";
  const rawSlug = typeof (parsed as { slug?: unknown }).slug === "string"
    ? (parsed as { slug: string }).slug
    : slugFromPath(metadataPath, fallbackSlug);
...
export const toLocalRelease = async (
  ...
  const version = metadata.version && metadata.version.length > 0
    ? metadata.version
    : `0.0.0-dev.${Date.now()}`;
