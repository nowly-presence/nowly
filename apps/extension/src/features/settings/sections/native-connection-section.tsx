...
export const NativeConnectionSection = ({
  activity,
  presences,
  userScripts,
  nativeStatus,
          ...
          <div className="flex min-w-0 items-center gap-2 text-sm text-foreground">
            <span
              className={cn(
                ...
                healthy ? "bg-accent" : nativeStatus.connected ? "bg-warning" : "bg-destructive",
              )}
            />
            <span className="truncate">
              {healthy
                ? t("status-bar-discord-connected")
                : nativeStatus.connected
                  ? t("status-bar-discord-closed")
                  : t("status-bar-host-missing")}
            </span>
          </div>
          <Badge
            variant="outline"
      ...
      {hostVersionInfo?.updateAvailable ? (
        <div className="rounded-xl border border-accent/20 bg-accent/5 p-3">
          ...
          <div className="flex flex-col gap-2 sm:flex-row">
            <Button
              variant="outline"
              className="flex-1 border-accent/20 bg-accent/10 text-accent hover:bg-accent/20"
              render={
                <a
