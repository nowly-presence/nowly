import { getCategoryLabel, type PresenceCategory } from "@/features/presences/presence-list.model";
...
export const toStorePresence = (item: PresenceCatalogItem): StorePresence => {
  ...
  return {
    ...
    color: typeof item.color === "string" && item.color.length > 0 ? item.color : "#5865F2",
...
export const filterStorePresences = (
  items: StorePresence[],
  query: string,
  category: PresenceCategory | null,
): StorePresence[] => {
