...
import { IconCheck, IconX } from "@/lib/tabler-icons";
...
export const AnalyticsChecklist: FC = () => (
  <div className="mt-5 grid grid-cols-2 gap-1.5 text-left">
    {notCollectedItems.map((key) => (
      <div key={key} className="flex items-center gap-2 rounded-xl border border-border bg-card-2 px-2.5 py-2">
        <IconX className="h-3.5 w-3.5 shrink-0 text-red-400" />
        <span className="text-xs text-muted-foreground">{t(key)}</span>
      </div>
    ))}
    <div className="col-span-2 flex items-center justify-center gap-2 rounded-xl border border-border bg-card-2 px-2.5 py-2">
