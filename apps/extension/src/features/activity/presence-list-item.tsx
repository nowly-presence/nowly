...
export const PresenceListItem = ({ onOpen, onUpdatePresence, updating = false, onSchedule, onToggle, presence, showSchedule, slug, updateAvailable }: Props): React.JSX.Element | null => {
    ...
    <article className="relative bg-card transition-colors first:rounded-t-xl last:rounded-b-xl hover:bg-secondary">
      {updateAvailable ? (
        <div className="flex h-8 items-center gap-2 border-b border-border bg-card px-1.5">
          ...
          <button type="button" onClick={openUpdate} className="min-w-0 flex-1 truncate text-left text-[11px] font-medium text-muted-foreground transition-colors hover:text-foreground">
            {t("presence-update-available")}
          </button>
          <Button variant="outline" size="xs" onClick={openUpdate} disabled={updating}>
      ...
      <div className="flex items-center gap-3 px-3 py-3">
        <button type="button" onClick={() => onOpen(slug)} className="flex min-w-0 flex-1 items-center gap-3 text-left">
          <PresenceTile slug={slug} name={presence.metadata.name} dimmed={!presence.enabled} className="size-10" />

          <div className="min-w-0 flex-1">
            <p className={presence.enabled ? "truncate text-sm font-medium text-foreground" : "truncate text-sm font-medium text-muted-foreground/80"}>{presence.metadata.name}</p>
            <div className="flex min-w-0 items-center gap-2 text-xs text-muted-foreground">
              <span className="h-1.5 w-1.5 rounded-full" style={{ backgroundColor: presence.enabled ? presence.metadata.color : "var(--muted-foreground)" }} />
        ...
        {showSchedule ? (
          <Button variant="ghost" size="icon-sm" onClick={() => onSchedule(slug)} aria-label={t("schedule")}>
            <RiCalendarLine />
          </Button>
        ) : null}

        <Button variant="ghost" size="icon-sm" onClick={() => onOpen(slug)} aria-label={t("settings")} title={t("settings")}>
          <RiSettings3Line />
        </Button>
      </div>
    </article>
